...
class LocalEnhancedFeedForward(nn.Module):
    def __init__(self, dim, expansion_ratio=4, kernel_size=3, dropout=0.):
        super().__init__()
        self.expansion = nn.Sequential(
            nn.Conv1d(dim, dim * expansion_ratio, 1),
            nn.GELU(),
            nn.Dropout(dropout)
        )
        self.conv = nn.Sequential(
            nn.Conv1d(dim * expansion_ratio, dim, kernel_size, padding=kernel_size//2),
            nn.Dropout(dropout)
        )

    def forward(self, x):
        out = self.expansion(x.transpose(1, 2)).transpose(1, 2)
    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                LocalEnhancedFeedForward(dim, dropout=dropout)
            ]))
...
class CoAtViT(nn.Module):
    ...
    def forward(self, img):
        ...
        # Apply convolutional layer after transformer block
        print(x.shape)
