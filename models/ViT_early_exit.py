...
class Transformer(nn.Module):
    ...
    def forward(self, x, confidence_threshold=0.9):
        early_exit_info = {'exited': False, 'confidence': 0.0}
        ...
        for idx, (attn, ff) in enumerate(self.layers):
            ...
            if self.early_exit and idx == self.depth - 2:
                probabilities = self.mlp_head(x[:,0])
                probabilities = torch.softmax(x[:,0], dim=-1)
    ...
    # def forward(self, x, confidence_threshold=0.9):
    #     early_exit_info = {'early_exit_count': 0}
    #     batch_size = x.size(0)
    ...
    #         if mask.any():
    #             x_attn = attn(x[mask])
    #             x[mask] = x[mask] + x_attn
    #             x[mask] = x[mask] + ff(x[mask])

    #             if self.early_exit and idx == self.depth - 2:
    #                 # x_mean = x[mask].mean(dim=1)
    #                 x = x[mask][:,0]
    #                 logits = self.mlp_head(x)
    #                 probabilities = torch.softmax(logits, dim=-1)
    #                 max_probability, predicted = torch.max(probabilities, dim=-1)
    #                 confidence = max_probability > confidence_threshold

    #                 if confidence.any():
    #                     early_exit_info['early_exit_count'] += confidence.sum().item()
...
class ViT_early_exit(nn.Module):
    ...
    def forward(self, img):
        ...
        x,early_exit_info=self.transformer(x)#,early_exit_info = self.transformer(x)
